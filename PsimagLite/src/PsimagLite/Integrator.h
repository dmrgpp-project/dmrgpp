...
public:
	...
	Integrator(FunctionType& function,
	    ...
	    , result_(0)
	    , abserr_(0)
	    , workspace_(gslWrapper_.gslIntegrationWorkspaceAlloc(limit_ + 2))
	{
	...
	~Integrator() { gslWrapper_.gslIntegrationWorkspaceFree(workspace_); }
	...
	{
		int status = gslWrapper_.gslIntegrationQagiu(
	...
	RealType operator()()
	{
		int status = gslWrapper_.gslIntegrationQagi(
...
private:
	...
	{
		int status = gslWrapper_.gslIntegrationQagp(&f_,
		                                            &(pts[0]),
		                                            pts.size(),
		                                            epsabs_,
		                                            epsrel_,
	...
	{
		int status = gslWrapper_.gslIntegrationQag(&f_,
		                                           pts[0],
		                                           pts[1],
		                                           epsabs_,
		                                           epsrel_,
	...
	RealType                                 result_;
	RealType                                 abserr_;
	GslWrapperType::GslIntegrationWorkspace* workspace_;
	GslWrapperType::GslFunction              f_;
};
} // namespace PsimagLite
