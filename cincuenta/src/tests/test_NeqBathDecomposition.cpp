...
{
	PsimagLite::Vector<double>::Type bath_params = { 0.0, 0.0 };
                                  ...
                                  const std::vector<std::vector<double>>& target)
{
	...
	for (SizeType n = 0; n <= nT; ++n) {
		for (SizeType j = 0; j <= nT; ++j) {
			...
			max_err = std::max(max_err, std::abs(val.real() - target[n][j]));
		}
	}
	return max_err;
}
...
{
	const SizeType n_t = 6;
	for (SizeType l : { 1u, 2u, 3u }) {
		auto target = makePsdTarget(n_t, static_cast<int>(l));
		auto delta  = makeDelta(n_t, target);
		auto decomp = makeDecomp(l, n_t);
		runUpdates(decomp, delta, n_t);
...
TEST_CASE("L=2: column 0 is alive after fix", "[NeqBathDecomposition][seeding]")
{
	...
	const SizeType n_t    = 6;
	auto           target = makePsdTarget(n_t, 2);
	auto           delta  = makeDelta(n_t, target);
	auto           decomp = makeDecomp(2, n_t);
	runUpdates(decomp, delta, n_t);
...
{
	const SizeType n_t    = 8;
	auto           target = makePsdTarget(n_t, 3);
	auto           delta  = makeDelta(n_t, target);
	auto           decomp = makeDecomp(3, n_t);
	runUpdates(decomp, delta, n_t);
...
{
	const SizeType n_t    = 10;
	auto           target = makePsdTarget(n_t, 1);
	auto           delta  = makeDelta(n_t, target);
	auto           decomp = makeDecomp(1, n_t);
	runUpdates(decomp, delta, n_t);

	// A rank-1 Cholesky of a rank-1 matrix should be exact up to floating point.
	CHECK(reconstructionError(decomp, n_t, target) < 1e-10);
}
...
TEST_CASE("L=2 reconstructs a rank-2 matrix accurately", "[NeqBathDecomposition][reconstruction]")
{
	...
	CHECK(reconstructionError(decomp, n_t, target) < 1e-8);
}
          ...
          "[NeqBathDecomposition][reconstruction]")
{
	...
	auto decomp1 = makeDecomp(1, n_t);
	runUpdates(decomp1, delta, n_t);
	const double err1 = reconstructionError(decomp1, n_t, target);

	auto decomp2 = makeDecomp(2, n_t);
	runUpdates(decomp2, delta, n_t);
	const double err2 = reconstructionError(decomp2, n_t, target);
...
{
	const SizeType n_t    = 30;
	const double   dt     = 0.2;
	const double   decay  = 0.15;
	auto           target = makeFullRankTarget(n_t, dt, decay);
	auto           delta  = makeDelta(n_t, target);
	auto           decomp = makeDecomp(3, n_t);
	runUpdates(decomp, delta, n_t);

	auto diag_at = [&](SizeType n)
	{
		std::complex<double> val(0);
		for (SizeType p = 0; p < decomp.rank(); ++p)
			val += decomp.vplus(static_cast<int>(n), static_cast<int>(p))
	...
	CHECK(diag_at(1) == Catch::Approx(0.5000000000000001).epsilon(1e-6));
	CHECK(diag_at(5) == Catch::Approx(0.49021304302869706).epsilon(1e-6));
	CHECK(diag_at(10) == Catch::Approx(0.4771665196067349).epsilon(1e-6));
	CHECK(diag_at(15) == Catch::Approx(0.4637717267240376).epsilon(1e-6));
	CHECK(diag_at(20) == Catch::Approx(0.45078669197353066).epsilon(1e-6));
	CHECK(diag_at(25) == Catch::Approx(0.437773730548557).epsilon(1e-6));
	CHECK(diag_at(30) == Catch::Approx(0.42485714581863027).epsilon(1e-6));
          ...
          "[NeqBathDecomposition][reconstruction][incisive]")
{
	...
	auto           target = makeComplexPhaseTarget(n_t, dt, decay, omega);
	auto           delta  = makeComplexDelta(n_t, target);
	auto           decomp = makeDecomp(3, n_t);
	runUpdates(decomp, delta, n_t);
